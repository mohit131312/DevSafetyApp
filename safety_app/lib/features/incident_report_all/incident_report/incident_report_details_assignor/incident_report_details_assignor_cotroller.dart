...
class IncidentReportDetailsAssignorCotroller extends GetxController {
  ...
  FocusNode assignorcommentFocusnode = FocusNode();
  ...
  Future getIncidentReportAssignorDetails(
    ...
    try {
      ...
      log(safetyIncidentReport[0].status.runtimeType.toString());
      log(safetyIncidentReport[0].status.toString());
      await safetyIncidentReport[0].status == 2
      ...
      if (userFound.value) {
        ...
        savedSignatureUrlfetch.value =
            safetyIncidentReport[0].signaturePhotoAfter.toString();
        log('safetyIncidentReport[0].assignerComment: ${safetyIncidentReport[0].assignerComment}');
        log('safetyIncidentReport[0].signaturePhotoAfter: ${safetyIncidentReport[0].signaturePhotoAfter}');
      } else {
        savedSignatureUrlfetch.value = '';
      }
  ...
  Future<void> safetySaveIncidentAssignorComment(
      BuildContext c, incidentId, status) async {
    log("status: $status");
    try {
      showDialog(
          context: c,
      ...
      var request = await http.MultipartRequest(
          "POST",
          Uri.parse(
              '${RemoteServices.baseUrl}save_safety_incident_assigner_comment'));
...
//---------------------------------------------------------
  ...
  void resetIncidentAssignorData() {
    // Clear form inputs and signature
    resetAssigneeForm();

    // Clear fetched backend data
    resetData();
    ...
    // Reset scroll controllers to top

    // Clear any remaining input
    incidentReportController.clear();

    log("✅ Incident assignor data has been fully reset.");
  }
}
