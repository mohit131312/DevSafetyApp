...
class ToolboxTrainingReviewerController extends GetxController {
  ...
  FocusNode reviwerFOcusNode = FocusNode();
  ...
  Future gettoolBoxreviewerDetails(
      projcetId, userId, userType, toolBoxId) async {
    try {
      ...
      //-------------------------------------------------
      safetyToolboxReviwerTraining = (await data['safety_toolbox_training']
              as List<dynamic>)
      ...
      reviewerController.text =
          safetyToolboxReviwerTraining[0].reviwerComment ?? '';
      // ignore: unrelated_type_equality_checks
      safetyToolboxReviwerTraining[0].status == 1 ||
              safetyToolboxReviwerTraining[0].status == 2
          ? userFound.value = true
          : userFound.value = false;
      print('status ----${userFound.value}');

      savedSignatureUrlfetch.value =
          safetyToolboxReviwerTraining[0].reviwerPhotoSignatureAfter ?? '';
      log('savedSignatureUrlOrBase64: ${savedSignatureUrlfetch.value}');

      log('----------=safetyToolboxTraining: ${(safetyToolboxReviwerTraining.length)}');
  ...
  Future<void> safetySaveReviewerComment(
    BuildContext c,
    tool,
  ) async {
    try {
      ...
      request.fields['reviwer_comment'] = reviewerController.text.toString();

      request.files.add(await http.MultipartFile.fromPath(
        'reviwer_photo_signature_after',
        signatureFile.path,
      ));
  ...
  void cleartoolboxComment() {
    ...
    log("ToolboxTrainingReviewerController: All data has been reset.");
  }
}
