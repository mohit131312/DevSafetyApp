...
class AddLabourScreen extends StatelessWidget {
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            if (await CheckInternet.checkInternet()) {
                              ...
                              if (addLabourController.searchType.value ==
                                  'ID') {
                                print(
                                    '---------${addLabourController.searchType.value}');
                                ...
                                if (addLabourController
                                    ...
                                    addLabourController.searchResults.length >
                                        1) {
                                  ...
                                  await addLabourController
                                      .getSafetyLabourDetails(value, context,
                                          false, false, projectId);
                                  Navigator.pop(context);
                                }
                              }
                              addLabourController.searchFocusNodeAll.unfocus();
                              addLabourController.searchController.clear();
                            } else {
                              await showDialog(
                                context: Get.context!,
                                builder: (BuildContext context) {
                    ...
                    Row(
                      ...
                      children: [
                        GestureDetector(
                          key: addLabourController.profilePhotoKey,
                          onTap: () {
                            addLabourController.pickImage(context);
                          },
                          child: Stack(
                            children: [
                              Stack(
                    ...
                    Form(
                      key: formKey,
                      // autovalidateMode:
                      //     AutovalidateMode.onUserInteraction, // 👈 Important
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Obx(
                            () => AppNormalDropdown(
                              key: addLabourController.literacyKey,
                              enabled: !addLabourController.userFound
                                  .value, // ✅ Editable only if user NOT found
                          ...
                          Obx(
                            () => AppNormalDropdown(
                              key: addLabourController.maritalStatusKey,
                              enabled: !addLabourController.userFound
                                  .value, // ✅ Editable only if user NOT found
                          ...
                          Obx(
                            () => AppSearchDropdown(
                              key: addLabourController.bloodGroupKey,
                              enabled: !addLabourController.userFound
                                  .value, // ✅ Editable only if user NOT found
                          ...
                          Obx(
                            () => AppSearchDropdown(
                              key: addLabourController.reason,
                              items:
                          ...
                          Obx(
                            () => AppTextFormfeild(
                              ...
                              focusNode: addLabourController
                                  .contactFocusNode, // <-- here
                          ...
                          Container(
                            key: addLabourController.currentAdress,
                          ...
                          Container(
                            key: addLabourController.permamntAddress,
                            padding: EdgeInsets.only(
                          ...
                          Obx(
                            () => AppTextFormfeild(
                              ...
                              validator: (value) {
                                // Skip validation if userFoundrelation is true
                                if (addLabourController
                                    .userFoundrelation.value) {
                                  return null;
                                }
                          ...
                          // Obx(
                          //   () => AppTextFormfeild(
                          //     controller: addLabourController
                          ...
                          Obx(
                            () => AppSearchDropdown(
                              key: addLabourController.relationkey,
                              items: [
                                'Father',
                                'Mother',
                                'Brother',
  ...
  void validateAndFocusFirstInvalidField() {
    ...
    if (addLabourController.profilePhoto.value.isEmpty) {
      scrollToWidget(addLabourController.profilePhotoKey);
      return;
    }
    if (addLabourController.labournameController.text.trim().isEmpty) {
      addLabourController.fullnameFocusNode.requestFocus();
    }
    //}
    if (addLabourController.selectedLiteratre.value.isEmpty) {
      scrollToWidget(addLabourController.literacyKey);
      return;
    }

    if (addLabourController.selectedmarried.value.isEmpty) {
      scrollToWidget(addLabourController.maritalStatusKey);
      return;
    }

    if (addLabourController.selectedBloodGroup.value.isEmpty) {
      scrollToWidget(addLabourController.bloodGroupKey);
      return;
    }
    if (addLabourController.selectedreasons.value.isEmpty) {
      scrollToWidget(addLabourController.reason);
      return;
    }
    if (addLabourController.dateController.text.trim().isEmpty) {
      scrollToWidget(addLabourController.dob);
      return;
    }
    if (addLabourController.contactnumberController.text.trim().isEmpty) {
      addLabourController.contactFocusNode.requestFocus();
      return;
    }
    for (int i = 0; i < addLabourController.addressControllers.length; i++) {
      if (addLabourController.addressControllers[i].text.trim().isEmpty) {
        scrollToWidget(addLabourController.currentAdress);
        return;
      }
    }

    // Validate Current Address Dropdowns
    if (addLabourController.selectedState.value.isEmpty) {
      scrollToWidget(addLabourController.currentAdress);
      return;
    }
    if (addLabourController.selectedDistrict.value.isEmpty) {
      scrollToWidget(addLabourController.currentAdress);
      return;
    }
    for (int i = 0;
        i < addLabourController.permanantAddressController.length;
        i++) {
      if (addLabourController.permanantAddressController[i].text
          .trim()
          .isEmpty) {
        scrollToWidget(addLabourController.permamntAddress);
        return;
      }
    }

    // Validate Current Address Dropdowns
    if (addLabourController.selectedPermanantState.value.isEmpty) {
      scrollToWidget(addLabourController.permamntAddress);
      return;
    }
    if (addLabourController.selectedPermanantDistrict.value.isEmpty) {
      scrollToWidget(addLabourController.permamntAddress);
      return;
    }
    if (addLabourController.econtactnameController.text.trim().isEmpty) {
      addLabourController.econtactnameFocusNode.requestFocus();
      return;
    }

    if (addLabourController.econtactnumberController.text.trim().isEmpty) {
      addLabourController.econtactnumberFocusNode.requestFocus();
      return;
    }

    if (!RegExp(r'^\d{10}$')
        .hasMatch(addLabourController.econtactnumberController.text.trim())) {
      addLabourController.econtactnumberFocusNode.requestFocus();
      return;
    }

    if (addLabourController.selectedRelation.value.isEmpty) {
      scrollToWidget(addLabourController.relationkey);
      return;
    }
  }
